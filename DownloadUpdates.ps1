...
################
$windows = "$($env:windows)"
$windows_vista_x86 = "windowsvistax86"
$windows_vista_x64 = "windowsvistax64"
...
$folder_cwd_windows_vista_updates = "$($folder_cwd_windows_vista)\Updates"
...
if ($windows -eq $windows_vista_x86) {
	...
	if(Test-Path -Path "$folder_cwd_windows_vista"){
		$folder_cwd_windows_vista_new_name = "$($folder_cwd_windows_vista)_$(([TimeSpan] (Get-Date).ToLongTimeString()).TotalMilliseconds)"
		
		Write-Host " "
		Write-Host "The folder ""$folder_cwd_windows_vista"" already exists!"
		Write-Host "Renaming it from ""$folder_cwd_windows_vista"" to ""$($folder_cwd_windows_vista_new_name)""."
		
		Rename-Item -Path "$folder_cwd_windows_vista" -NewName "$($folder_cwd_windows_vista_new_name)"

		mkdir "$($folder_cwd_windows_vista)"
	}else{
		mkdir "$($folder_cwd_windows_vista)"
	}

	if(-Not (Test-Path -Path "$folder_cwd_windows_vista_updates")){
		mkdir "$($folder_cwd_windows_vista_updates)"
	}
